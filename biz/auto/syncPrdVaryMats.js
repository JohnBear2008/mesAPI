...
//	    	console.log("connectionMES:"+JSON.stringify(connectionMES))
    ...
    var getsql="select a.varyid,a.SubProdID,b.Action from prdVaryMats a,(select  PKValue,Action from comChangeLog where   changetime>= DATEADD(minute,-5, GETDATE()+2)  and ProgID='CHIProdt.VaryBom')b where a.varyid=b.PKValue";
...
function ERPtoMES(){
	...
	yjDBService_sqlserver.exec({
        ...
        success : function(result) {
            ...
            console.log("get 工程变更子表data:"+JSON.stringify(data));
            //插入mes-----------------
...
//            		console.log("SynchType:"+SynchType);
 //           		console.log("CreateTime:"+JSON.stringify(CreateTime));
            		...
            		yjDBService.exec({
                        ...
                        sql: postsql,
                        parameters: ["ERP","prdVaryMats",data[i].varyid,data[i].SubProdID,"API","0",SynchType,CreateTime],
                        success: function(result) {
                        	console.log("工程变更子表插入成功!")
                        },
                        error: {}
                    });
            	}
