...
//	    	console.log("connectionMES:"+JSON.stringify(connectionMES))
    ...
    var getsql=" select a.TakeMatNo,a.Flag,b.Action from prdTakeMaterial a,(select  PKValue,Action from comChangeLog where   changetime>= DATEADD(minute,-5, GETDATE()+2)  and ProgID='ChiProdt.TakeMat')b where a.TakeMatNo=b.PKValue";
...
function ERPtoMES(){
	...
	yjDBService_sqlserver.exec({
        ...
        success : function(result) {
            ...
            console.log("get领料退料单data:"+JSON.stringify(data));
            //插入mes-----------------
...
//            		console.log("SynchType:"+SynchType);
 //           		console.log("CreateTime:"+JSON.stringify(CreateTime));
            		...
            		yjDBService.exec({
                        ...
                        sql: postsql,
                        parameters: ["ERP","prdTakeMaterial",data[i].TakeMatNo,data[i].TakeMatNo,"API",data[i].Flag,SynchType,CreateTime],
                        success: function(result) {
                        	console.log("领料退料单插入成功!")
                        },
                        error: {}
                    });
            	}
